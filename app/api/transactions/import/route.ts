...
export async function POST(req: NextRequest) {
  try {
    ...
    if (file.type === "application/pdf" || fileName.endsWith(".pdf")) {
      ...
      // tenta pegar ano do cabeçalho
      const headerYearMatch = data.text.match(/20\d{2}/);
      if (headerYearMatch) {
        currentYear = parseInt(headerYearMatch[0]);
      }
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // 🔥 Detecta data em múltiplos formatos
        let date: Date | null = null;
    
        // formato 04/03/2026
        const fullDate = trimmed.match(/\d{2}\/\d{2}\/\d{4}/);
        if (fullDate) {
          date = parseDate(fullDate[0]);
        }
    ...
    /* ================= OFX ================= */
      ...
      const ofxTransactions =
        ...
        ofxData.statement?.transactions ??
        [];
    ...
    return NextResponse.json(
      { error: "Erro interno ao importar transações" },
      { status: 500 }
    );
  }
}
